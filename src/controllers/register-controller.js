...
export default ({ config, db }) => {
  ...
  api.get('/user/:id/history', validateToken, authenticate, (req, res) => {
    ...
    Register.find({ $and: [{ user: userId }, { buyAt: { $gte: initDate, $lte: lastDate } }] }, (err, result) => {
      let mapeadoErdernado = _.orderBy(
        _.map(result, item => {
          return {
            buyAt: item.buyAt,
            local: item.local,
...
function createCriteria(month, year, card, user, notIncludeCards) {
  let criteria = [{ installments: { $elemMatch: { paymentMonth: month, paymentYear: year } } }];

  if (notIncludeCards === 'true') {
    const compareDate = new Date(year, MONTHS.indexOf(month), 20);
    criteria = [{ installments: { $elemMatch: { paymentMonth: month, paymentYear: year, paymentDate: { $lt: compareDate } } } }];
  }
...
function getCriteriaParams(req) {
  ...
  const notIncludeCards = req.query.notIncludeCards;
  return {
    month,
    year,
    cardId,
